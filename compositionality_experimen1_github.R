...
#save.image("compos_ex1.Rdata")
load("compos_ex1.Rdata")
...
treatmentdata <- round(cbind(absdata,absdata))
for(i in 21:40){
...
for(k in c(1:100)){ 
  for(i in c(0.1,0.5,1,5,10)){
    for(j in c(0.01,0.05,0.1,0.25,0.5,0.75)){ 
      ...
      treatmentdatalist[[paste(k,i,j)]][1:(100*j),31:40] <- round(treatmentdatalist[[paste(k,i,j)]][1:(100*j),31:40]*(1+rnorm(n=10,i,i/10)))
...
for(h in 4:10){
  print(mean(treatmentdatalist[[paste(k,i,j)]][1:(100*j),(h+30)]/
               treatmentdata[1:(100*j),(h+10)]))
}

treatmentdatalist[[paste(k,i,j)]][1:(100*j),33]/
treatmentdata[1:(100*j),13]
...
windows(width=3,height = 4.5);
plot(log(t(treatmentdatalist[[paste(k,i,j)]])[meta$treatment=="tr",1])~meta$timepoint[meta$treatment=="tr"],
     col=1:10,pch=20,cex=2,xlim=c(0.5,2.5),xlab="Timepoint",ylab="Log abundance of feature 1",
     main="Treatment group 10%",xaxt="n")
axis(side=1,at=c(1,2))
for(h in 1:10){
  lines(log(t(treatmentdatalist[[paste(k,i,j)]])[meta$treatment=="tr",1][c(h,h+10)])~meta$timepoint[meta$treatment=="tr"][c(h,h+10)],
        col=h,lwd=2)
}
...
gt_bw[[paste(k,i,j)]][gt_bw[[paste(k,i,j)]]$taxon==h,"p"]<-kruskal.test(g=delta[,"treatment"],x=delta[,paste("V",h,sep="")])$p.value
}
}
}
}
...
windows(height=4);barplot(as.matrix(treatmentdatalist[['3 10 0.75']][rev(order(rowMeans(treatmentdatalist[['3 10 0.75']]))),1:40]),border=F,cex.names = 0.5,col=1:100,las=2)
windows(height=4);barplot(as.matrix(treatmentdatalist[['3 5 0.75']][rev(order(rowMeans(treatmentdatalist[['3 5 0.75']]))),21:40]),border=F,cex.names = 0.5,col=1:100,las=2)
...
caps <- capscale(peardist_log(t(treatmentdatalist[[paste(k,i,j)]]))~1)
#windows();plot(summary(caps)$sites[,2]~summary(caps)$sites[,1],col=as.factor(meta$treatment),pch=20)
#windows();plot(summary(caps)$sites[,2]~summary(caps)$sites[,1],col=as.factor(meta$timepoint),pch=20)
windows();plot(summary(caps)$sites[meta$timepoint==2,2]~summary(caps)$sites[meta$timepoint==2,1],col=as.factor(meta$treatment[meta$timepoint==2]),pch=20)
ad <- adonis2(peardist_log(t(treatmentdatalist[[paste(k,i,j)]][meta$timepoint==2]))~meta$treatment[meta$timepoint==2])


windows();plot(hclust(peardist_log(t(treatmentdatalist[[paste(k,i,j)]]))))

  ad_abs<-list()
  ad_abs_bc<-list()
  for(k in 1:100){
ad_abs[[k]]<- data.frame(effsize=rep(c(0.1,0.5,1,5,10),each=6),Nchange=rep(c(0.01,0.05,0.1,0.25,0.5,0.75),7))
ad_abs_bc[[k]]<- data.frame(effsize=rep(c(0.1,0.5,1,5,10),each=6),Nchange=rep(c(0.01,0.05,0.1,0.25,0.5,0.75),7))
  }
...
ad_abs_mean <- data.frame(effsize=rep(c(0.1,0.5,1,5,10),each=6),Nchange=rep(c(0.01,0.05,0.1,0.25,0.5,0.75),7))
...
abss <- cbind(result_abs[[1]][,c(11,12)],result_abs[[2]][,c(11,12)])
for(k in 3:100){
  abss <- cbind(abss,result_abs[[k]][,c(11,12)])
}
result_abs_mean$false_pos_prop <- rowMeans(abss[,grep(names(abss),pattern="pos",value=T)])
result_abs_mean$false_neg_prop <- rowMeans(abss[,grep(names(abss),pattern="neg",value=T)])
...
barplot(as.matrix(treatmentdata[rev(order(rowMeans(treatmentdata))),1:20]),
...
for(k in 1:100){
  ...
  for(i in c(0.1,0.5,1,5,10)){
    for(j in c(0.01,0.05,0.1,0.25,0.5,0.75)){ 
      ...
      relfolds <- abs(log(folds[,"bw"]))
      fccut <-  quantile(relfolds[pvals$adj<0.1],0.5)
...
windows();hist(relfolds[1:75],breaks=30,xlim=c(0,2))
abline(v=quantile(abs(relfolds[pvals$adj<0.1]),0.5))
windows();hist(relfolds[76:100],breaks=30,xlim=c(0,2))

#prokbert for viral prediction
#phase variation in bacteria
#cobra toolbox

#Fig False Positive Rel Abundance
windows(width=4,height = 3);
...
#Fig False NEgative Rel Abundance
windows(width=4,height =3);
...
ggplot(result_rel_mean, aes(as.factor(effsize), as.factor(Nchange), fill= false_pos_prop_fc)) + 
  ...
  ylab("Proportion responding")+
  xlab("Effect size")+
  geom_text(aes(label = false_pos_prop_fc), color = "white", size = 4) +
  guides(fill = guide_colourbar(title = "False pos"))+
  scale_fill_gradient(low="white", high="red",limits=c(0,1)) 
...
fc_comp <- list()
for(k in 1:100){
  fc_comp[[k]] <- data.frame(effsize=rep(c(0.1,0.5,1,5,10),each=600),Nchange=rep(c(0.01,0.05,0.1,0.25,0.5,0.75),500))
for(i in c(0.1,0.5,1,5,10)){
  for(j in c(0.01,0.05,0.1,0.25,0.5,0.75)){ 
...
windows(width=4,height = 5);plot.new()
legend("topleft",legend=unique(fc_comp[[k]]$effsize),pch = 21, cex=2,
       pt.bg = scales::alpha(pal(5),0.75),col="white",bty="n")


windows(width=4,height = 5);plot.new()
legend("topleft",legend=unique(fc_comp[[k]]$Nchange),pch = 21, cex=2,
       pt.bg = scales::alpha(pal(6),0.75),col="white",bty="n")
...
for(k in c(1:100)){ 
  for(i in c(0.1,0.5,1,5,10)){
    for(j in c(0.01,0.05,0.1,0.25,0.5,0.75)){  
      clrdatalist[[paste(k,i,j)]] <- t(clr(t(seqdatalist[[paste(k,i,j)]])))
  }}
}
...
for(k in 1:100){
 for(i in c(0.1,0.5,1,5,10)){
    for(j in c(0.01,0.05,0.1,0.25,0.5,0.75)){ 
      fc_comp[[k]]$FC_clr[fc_comp[[k]]$effsize==i&fc_comp[[k]]$Nchange==j]<- FC_clr[[paste(k,i,j)]]$bw
    }}}
...
ord = findInterval(fc_comp[[k]]$Nchange, sort(fc_comp[[k]]$Nchange))
windows(width=4,height = 4);
plot(log(FC_clr)~log(FC),data=fc_comp[[k]],pch=20,col=scales::alpha(pal(nrow(fc_comp[[k]]))[ord],0.75))
legend("topleft",legend=unique(fc_comp[[k]]$Nchange),pch = 21, pt.bg = scales::alpha(pal(6),0.75),cex=1,col="white",bty="n")


#deseq data--------------------------------------------------------------
dsdata<-as.data.frame(deseqdatalist[1])
windows();barplot(as.matrix(dsdata[rev(order(rowMeans(absdata))),]),border=F,cex.names = 0.8,col=1:100,las=2)
...
for(k in 1:100){
  for(i in c(0.1,0.5,1,5,10)){
    for(j in c(0.01,0.05,0.1,0.25,0.5,0.75)){ 
      fc_comp[[k]]$FC_deseq[fc_comp[[k]]$effsize==i&fc_comp[[k]]$Nchange==j]<- FC_deseq[[paste(k,i,j)]]$bw
    }}}
...
ord = findInterval(fc_comp[[k]]$Nchange, sort(fc_comp[[k]]$Nchange))
windows(width=4,height = 4);
plot(log(FC_deseq)~log(FC),data=fc_comp[[k]],pch=20,col=scales::alpha(pal(nrow(fc_comp[[k]]))[ord],0.75))
legend("topleft",legend=unique(fc_comp[[k]]$Nchange),pch = 21, pt.bg = scales::alpha(pal(6),0.75),cex=1,col="white",bty="n")
...
for(k in 1:100){
  ...
  for(i in c(0.1,0.5,1,5,10)){
    for(j in c(0.01,0.05,0.1,0.25,0.5,0.75)){ 
      ...
      result_edger[[k]]$discovered_pos[result_edger[[k]]$effsize==i&result_edger[[k]]$Nchange==j] <- nrow(pvals[pvals[,"adj"]<0.1,])
      result_edger[[k]]$discovered_neg[result_edger[[k]]$effsize==i&result_edger[[k]]$Nchange==j] <- nrow(pvals[pvals[,"adj"]>0.1,])
      result_edger[[k]]$correct_pos[result_edger[[k]]$effsize==i&result_edger[[k]]$Nchange==j] <- nrow(pvals[1:(100*j),][pvals[1:(100*j),"adj"]<0.1,])
      result_edger[[k]]$false_pos[result_edger[[k]]$effsize==i&result_edger[[k]]$Nchange==j] <- nrow(pvals[(100*j+1):100,][pvals[(100*j+1):100,"adj"]<0.1,])
      result_edger[[k]]$false_neg[result_edger[[k]]$effsize==i&result_edger[[k]]$Nchange==j] <- nrow(pvals[1:(100*j),][pvals[1:(100*j),"adj"]>0.1,])
      result_edger[[k]]$correct_neg[result_edger[[k]]$effsize==i&result_edger[[k]]$Nchange==j] <- nrow(pvals[(100*j+1):100,][pvals[(100*j+1):100,"adj"]>0.1,])
    }}
...
for(k in 1:100){
  for(i in c(0.1,0.5,1,5,10)){
    for(j in c(0.01,0.05,0.1,0.25,0.5,0.75)){ 
      fc_comp[[k]]$FC_edger[fc_comp[[k]]$effsize==i&fc_comp[[k]]$Nchange==j]<- FC_edger[[paste(k,i,j)]]$bw
    }}}
...
ord = findInterval(fc_comp[[k]]$Nchange, sort(fc_comp[[k]]$Nchange))
windows(width=4,height = 4);
plot(log(FC_edger)~log(FC),data=fc_comp[[k]],pch=20,col=scales::alpha(pal(nrow(fc_comp[[k]]))[ord],0.75))
legend("topleft",legend=unique(fc_comp[[k]]$Nchange),pch = 21, pt.bg = scales::alpha(pal(6),0.75),cex=1,col="white",bty="n")


ord = findInterval(result_deseq_mean$Nchange, sort(result_edger_mean$Nchange))
windows();plot(false_pos_prop~I(1-false_neg_prop),data=result_edger_mean,ylab="False positive rate",xlab="True positive rate",
              pch=20,col=scales::alpha(pal(nrow(result_edger_mean))[ord],0.75),cex=2)
...
for(k in c(1:100)){ 
  for(i in c(0.1,0.5,1,5,10)){
    for(j in c(0.01,0.05,0.1,0.25,0.5,0.75)){ 
      contdatalist[[paste(k,i,j)]] <- reldatalist[[paste(k,i,j)]]
      for(f in 1:100){
      contdatalist[[paste(k,i,j)]][f,meta$timepoint==2] <- reldatalist[[paste(k,i,j)]][f,meta$timepoint==2]/(mean(t(reldatalist[[paste(k,i,j)]])[meta$timepoint==2&meta$treatment=="c",f]/
                t(reldatalist[[paste(k,i,j)]])[meta$timepoint==1&meta$treatment=="c",f],na.rm=T))
      }
      }
  }
}
...
for(k in c(1:10)){ 
  for(i in c(0.1,0.5,1,5,10)){
    for(j in c(0.01,0.05,0.1,0.25,0.5,0.75)){ 
      tr1 <- as.data.frame(t(contdatalist[[paste(k,i,j)]])+0.0001)
      tr1$timepoint <- rep(c("1","2"),each=20)
      tr1$treatment <- rep(c("c","tr","c","tr"),each=10)
      delta <- (tr1[tr1$timepoint==2,1:100]-tr1[tr1$timepoint==1,1:100])/(tr1[tr1$timepoint==1,1:100])
      delta$treatment <- tr1$treatment[1:20]
      gt_bw_cont[[paste(k,i,j)]] <- data.frame(taxon=c(1:100),p=rep(NA,100))
      for(h in c(1:38,40:56,58:100)){
        gt_bw_cont[[paste(k,i,j)]][gt_bw_cont[[paste(k,i,j)]]$taxon==h,"p"]<-kruskal.test(g=delta[,"treatment"],x=delta[,paste("V",h,sep="")])$p.value
      }
    }
  }
}
...
result_cont <-list()
for(k in 1:10){
  result_cont[[k]] <- data.frame(effsize=rep(c(0.1,0.5,1,5,10),each=6),Nchange=rep(c(0.01,0.05,0.1,0.25,0.5,0.75),5))
  for(i in c(0.1,0.5,1,5,10)){
    for(j in c(0.01,0.05,0.1,0.25,0.5,0.75)){ 
      pvals <- as.data.frame(gt_bw_cont[[paste(k,i,j)]])
      pvals$adj <- p.adjust(pvals[,2],"fdr")
      result_cont[[k]]$true_pos[result_cont[[k]]$effsize==i&result_cont[[k]]$Nchange==j]<- j*100
      result_cont[[k]]$true_neg[result_cont[[k]]$effsize==i&result_cont[[k]]$Nchange==j]<- 100-(j*100)
      result_cont[[k]]$discovered_pos[result_cont[[k]]$effsize==i&result_cont[[k]]$Nchange==j] <- nrow(pvals[pvals[,"adj"]<0.1,])
      result_cont[[k]]$discovered_neg[result_cont[[k]]$effsize==i&result_cont[[k]]$Nchange==j] <- nrow(pvals[pvals[,"adj"]>0.1,])
      result_cont[[k]]$correct_pos[result_cont[[k]]$effsize==i&result_cont[[k]]$Nchange==j] <- nrow(pvals[1:(100*j),][pvals[1:(100*j),"adj"]<0.1,])
      result_cont[[k]]$false_pos[result_cont[[k]]$effsize==i&result_cont[[k]]$Nchange==j] <- nrow(pvals[(100*j+1):100,][pvals[(100*j+1):100,"adj"]<0.1,])
      result_cont[[k]]$false_neg[result_cont[[k]]$effsize==i&result_cont[[k]]$Nchange==j] <- nrow(pvals[1:(100*j),][pvals[1:(100*j),"adj"]>0.1,])
      result_cont[[k]]$correct_neg[result_cont[[k]]$effsize==i&result_cont[[k]]$Nchange==j] <- nrow(pvals[(100*j+1):100,][pvals[(100*j+1):100,"adj"]>0.1,])
    }}
  result_cont[[k]]$false_pos_prop <- round(result_cont[[k]]$false_pos/(result_cont[[k]]$false_pos+result_cont[[k]]$true_neg),digits=2)
  result_cont[[k]]$false_neg_prop <- round(result_cont[[k]]$false_neg/(result_cont[[k]]$true_pos+result_cont[[k]]$false_neg),digits=2)
}

result_cont_mean <-data.frame(effsize=rep(c(0.1,0.5,1,5,10),each=6),Nchange=rep(c(0.01,0.05,0.1,0.25,0.5,0.75),5))
conts <- cbind(result_cont[[1]][,c(11,12)],result_cont[[2]][,c(11,12)])
for(k in 3:100){
  conts <- cbind(conts,result_cont[[k]][,c(11,12)])
}
result_cont_mean$false_pos_prop <- rowMeans(conts[,grep(names(conts),pattern="pos",value=T)])
result_cont_mean$false_neg_prop <- rowMeans(conts[,grep(names(conts),pattern="neg",value=T)])


windows(width=4,height = 3);
ggplot(result_cont_mean, aes(as.factor(effsize), as.factor(Nchange), fill= false_pos_prop)) + 
  geom_tile(color = "white",
            lwd = 1.5,linetype = 1) +
  ylab("Proportion responding")+
  xlab("Effect size")+
...
windows(width=4,height =3);
ggplot(result_cont_mean, aes(as.factor(effsize), as.factor(Nchange), fill= false_neg_prop)) + 
  geom_tile(color = "white",
            lwd = 1.5,linetype = 1) +
  ylab("Proportion responding")+
  xlab("Effect size")+
